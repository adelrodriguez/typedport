...
- **`event()` is now `channel()`.** One leaf constructor for both shapes — `channel({ input, output })` is a round trip, `channel(schema)` is one-way — matching the vocabulary the library already used everywhere else (`router.channels`, `unknown-channel`). The leaf discriminant is now `_kind: "channel"`.
...
- The README now documents the adapter-author toolkit (`flatten`, `isLeaf`, `parseWith`, `Transport`, `OneWayContract`, `InferClient`, `Leaf`) and the client-side output-validation pattern (`parseWith(api.leaf.$output, result)`).
...
Migrating: rename `event(` → `channel(` and `TypeportError` → `ChannelError`; nothing else changes.
