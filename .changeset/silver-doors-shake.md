...
- New `typeport/wire` subpath export: `toWire`/`fromWire` are a symmetric codec for outcomes — `toWire(promiseOrThunk)` captures any operation's result or failure as a serializable envelope, `fromWire` unwraps it or rethrows (`ValidationError` crosses boundaries with its issues intact) — and `connect(wire, { router?, timeoutMs? })` turns any duplex message pipe into a symmetric transport with request/response correlation, per-call timeouts, and `close` teardown.
